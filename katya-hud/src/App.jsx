    ...
    return (
      <div style={setupContainerStyle}>
        ...
        <div style={{marginTop: "20px", width: "300px", display: "flex", flexDirection: "column", gap: "15px"}}>
          ...
          <input type="text" placeholder="CLASS" style={inputStyle} onChange={(e) => setFormData({...formData, profession: e.target.value})} />
  ...
  if (view === "DASHBOARD" && user) {
    ...
    return (
      <div style={{
          ...
          <div style={{display: "grid", gap: "10px"}}>
            {economy.quests.length === 0 ? (
               <div style={{opacity: 0.5, textAlign: "center", padding: "20px"}}>
                 NO QUESTS LOADED<br/>
                 <span style={{fontSize: "10px"}}>(System will generate default plan shortly)</span>
               </div>
            ) : (
               economy.quests.map(q => (
                <div key={q.id} style={{
                  ...
                  <div style={{textAlign: "right", fontSize: "14px", fontWeight: "bold"}}>
                    {q.completed ? <span>✅ DONE</span> : <span>{Math.round(q.current_minutes)} / {q.target_minutes}m</span>}
                  </div>
                </div>
              ))
            )}
        ...
        {isLocked && (
          <div className="blink" style={{marginTop: "30px", color: "red", textAlign: "center", fontWeight: "bold", background: "black", padding: "15px", border: "2px solid red"}}>
            ⛔ RECOVERY MODE ACTIVE ⛔<br/>EARN 100 CREDITS TO UNLOCK
          </div>
        )}
      </div>
    );
...
const setupContainerStyle = { height: "100vh", background: "#050505", color: "#00f3ff", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", fontFamily: "'Courier New', monospace" };
const inputStyle = { width: "300px", background: "transparent", border: "none", borderBottom: "1px solid #00f3ff", color: "white", padding: "8px", fontFamily: "inherit", outline: "none" };
const buttonStyle = { marginTop: "20px", background: "#00f3ff", color: "black", border: "none", padding: "10px", fontWeight: "bold", cursor: "pointer", width: "316px" };
